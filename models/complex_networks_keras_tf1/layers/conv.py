...
from .fft import ifft_func
# from .stft import ISTFT_network
...
class _ComplexConv(Layer):
    ...
    def call(self, inputs):
        ...
        if self.transposed:
            ...
            convFunc = {2: conv2d_transpose}[self.rank]
        else:
            convFunc = {
                1:  K.conv1d,
                2:  K.conv2d,
        ...
        if self.transposed:
            cat_kernels_4_complex._keras_shape = self.kernel_size + (2 * self.filters, 2 * input_dim)
        else:
