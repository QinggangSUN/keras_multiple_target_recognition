...
def DenseNet(input_shape=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
    ...
    if weights == 'imagenet':
        ...
        if weights_loaded:
            ...
            logging.info("Weights for the model were loaded successfully")
...
def __transition_block(ip, nb_filter, compression=1.0, weight_decay=1e-4):
    ...
    x = Conv2D(int(nb_filter * compression), (1, 1), kernel_initializer='he_normal', padding='same', use_bias=False,
               kernel_regularizer=l2(weight_decay))(x)
    # logging.debug('before transition_block averagepooling ', x)
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
    ...
    # logging.debug('before initial_block ', img_input)
    ...
    # logging.debug('after initial_block ', x)
    ...
    for block_idx in range(nb_dense_block - 1):
        ...
        # logging.debug('after dense_block ', x)
