# -*- coding: utf-8 -*-
"""
Created on Tue Feb  9 18:03:39 2021
...
if __name__ == '__main__':
    ...
    from loss_acc import subset_acc_int, subset_acc_int_np, macro_averaged_acc_int, macro_averaged_acc_int_np
    from prepare_data_shipsear_recognition_mix_s0tos3full3 import PathSourceRootFull
    ...
    def test_model(model, x_list, y_list, pbs, modelname, path_save, **kwargs):
        ...
        bool_evaluate = True
        bool_subset_acc_int = True
        bool_macro_averaged_acc_int = True

        if 'evaluate' in kwargs.keys():
            bool_evaluate = kwargs['evaluate']
        if 'subset_acc_int' in kwargs.keys():
            bool_subset_acc_int = kwargs['subset_acc_int']
        if 'macro_averaged_acc_int' in kwargs.keys():
            bool_macro_averaged_acc_int = kwargs['macro_averaged_acc_int']
        ...
        if bool_subset_acc_int:
            ...
            dict_r.update({'subset_acc_int_val': float(subset_acc_int_val)})
            dict_r.update({'subset_acc_int_test': float(subset_acc_int_test)})

        if bool_macro_averaged_acc_int:
            d0_train = x_train.shape[0]  # d0_train = number of train samples
            d0_val = x_val.shape[0]  # d0_val = number of val samples
            d0_test = x_test.shape[0]  # d0_test = number of test samples
            ...
            dict_r.update({'macro_averaged_acc_int_val': macro_averaged_acc_int_val.tolist()})
            dict_r.update({'macro_averaged_acc_int_test': macro_averaged_acc_int_test.tolist()})
    ...
    def test_check_models(path_save_model, x_list, y_list, num_model, pbs=256, kw_model='.hdf5',
        ...
        Example:
            ...
            test_check_models(path_save_model, X_LIST, Y_LIST, 5, dict_model_load={'subset_acc_int':subset_acc_int})
        """
        ...
        if mode_load in {2, 3}:
            ...
            loss = 'MSE' if 'loss' not in kwargs.keys() else kwargs['loss']
            metrics = [subset_acc_int] if 'metrics' not in kwargs.keys() else kwargs['metrics']
    ...
    def train_model(model, x_list, y_list, paras, path_save):
        ...
        model.compile(
            optimizer=optimizer, loss='MSE', metrics=metrics)
        ...
        check_filename = os.path.join(path_check, 'weights_'+modelname+'_{epoch:02d}_{val_subset_acc_int:.2f}.hdf5')
        checkpoint = ModelCheckpoint(filepath=check_filename, monitor='val_subset_acc_int', mode='max',
                                     verbose=1, period=1, save_best_only=True)
        ...
        output_history(
            ...
            os.path.join(path_history, 'loss_'+modelname+'.svg'), show=False,
            title='loss val_loss', label_x='epoch', label_y='loss', loc='upper left')

        output_history(
            ['b', 'y'], [history.history['subset_acc_int'], history.history['val_subset_acc_int']],
            os.path.join(path_history, f'subset_acc_int_{modelname}.svg'),
    ...
    def load_data(path_data=None, path_root=None, form_src='None', scaler_data='or', sub_set_way='rand', **kwargs):
        ...
        Args:
            ...
            path_root (str): if path_data is None, compute path_data through class PathSourceRootFull
    ...
    def search_models(x_list, y_list, model_list, path_save, **kwargs):
        ...
        if 90 in model_list:
            ...
            paras = {'i': i, 'j': j, 'epochs': 1, 'batch_size': 64, 'n_gpu': 1}
        ...
        if 12 in model_list:
            ...
            paras = {'i': i, 'j': j, 'epochs': 3, 'batch_size': 10, 'n_gpu': 1, 'test_check_models': False}
        ...
        if 10 in model_list:
            ...
            paras = {'i': i, 'j': j, 'epochs': 3, 'batch_size': 10, 'n_gpu': 1}
        ...
        if 15 in model_list:
            ...
            paras = {'i': i, 'j': j, 'epochs': 1, 'batch_size': 1, 'n_gpu': 1}
    ...
    # --------------------------------------------------------------------------
    DICT_MODEL_CONFIG = {'subset_acc_int': subset_acc_int, 'macro_averaged_acc_int': macro_averaged_acc_int}
    ...
    DICT_MODEL_COMPILE = {'optimizer': optimizers.Adam(lr=1e-3),
                          'loss': 'MSE',
                          'metrics': [subset_acc_int, macro_averaged_acc_int]}
    # --------------------------------------------------------------------------
    # for wavmat
    ...
    for win_i, hop_i in zip(WIN_LIST, HOP_LIST):
        for n_mels_i in N_MELS:
            ...
            mkdir(path_save)
            x_list, y_list = load_data(path_root=PATH_ROOT, form_src='logmelspectrum',
    ...
    for win_i, hop_i in zip(WIN_LIST, HOP_LIST):
        for n_mels_i in N_MELS:
            for n_mfcc_i in N_MFCCS:
                ...
                mkdir(path_save)
                x_list, y_list = load_data(path_root=PATH_ROOT, form_src='mfcc',
                                           scaler_data='or', sub_set_way='rand',
    ...
    for win_i, hop_i in zip(WIN_LIST, HOP_LIST):
        ...
        mkdir(path_save)
        real_x_list, y_list = load_data(path_root=PATH_ROOT, form_src='realspectrum',
                                        scaler_data='or', sub_set_way='rand',
                                        **{'win_length': win_i, 'hop_length': hop_i})
        real_x_list, y_list = standar_data(real_x_list, y_list, 2, 1, test_few=False)

        img_x_list, y_list = load_data(path_root=PATH_ROOT, form_src='imgspectrum',
                                       scaler_data='or', sub_set_way='rand',
                                       **{'win_length': win_i, 'hop_length': hop_i})
        ...
        for i in range(1, 2):
            for j in range(-3, -4, -1):
                search_models(x_list, y_list, [19], path_save, **{'i': i, 'j': j})  # 15, 16, 17, 18, 19
